...
module mod_cmdline
  ...
  type subfield_info
    character (len=55) :: name
    character (len=55) :: dataname
  end type
  type field_info
    character (len=55) :: full
    type(subfield_info), allocatable ,dimension(:)  :: subfield
  end type
  type cmd_line_arg
    character(2) :: switch
...
subroutine collect_args (accepted_switches)
  ...
  do i=1, ntokens(dummy)
    ...
    print * , trim(cmd_line(i)%full)
    cmd_line(i)%switch=cmd_line(i)%full(1:2)
    print * , trim(cmd_line(i)%switch) 
    ...
    do j=1,size(cmd_line(i)%field)
      ...
      do n = 1 , count_separator(cmd_line(i)%field(j)%full,":")+1
        ...
        print * , trim(cmd_line(i)%field(j)%subfield(n)%name) , &
          "__" ,  trim(cmd_line(i)%field(j)%subfield(n)%dataname )
      enddo
...
subroutine intro (program_calling, accepted_switches , cmdlineargs )
  ...
  !  call get_command_cleaned(dummy_cleaned)
  !
  !  !todo delete

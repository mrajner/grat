...
subroutine model_aliases(model, dryrun, year)
  ...
  case ("NCEP", "NCEP2", "NCEP1")
    ...
    case ("LS")
      ...
      write(model%name,'(a,a,i4,a)') &
        "/home/mrajner/dat/landsea/ncep_closed_seas_caspian.nc"
      model%autoload=.false.
    case default
      model%autoload=.false.
      model%if=.false.
  ...
  case ("ERA")
    ...
    case ("LS")
      model%names(1:3)=["z", "x", "y"]
      write(model%name,'(a,a,i4,a)') &
        "/home/mrajner/dat/landsea/era_closed_seas_caspian.nc"
      model%autoload=.false.
    case default
      model%autoload=.false.
      model%if=.false.
...
!! \date 2013-03-04
! =============================================================================
subroutine check(status, error, success)
  use netcdf
  use mod_printing
  use iso_fortran_env
  ...
  if(status /= nf90_noerr) then 
    call print_warning(trim(nf90_strerror(status)))
  ...
  if (present(error).and.(error)) stop "netCDF"

    ...
    do idist = 1, ubound(green_common(igreen)%distance,1)
      ...
      do iazimuth  = 1, nazimuth
        ...
        if (ind%model%ls.ne.0.and.inverted_barometer) then
          ...
          if (((val(ind%model%ls) - epsilon(1._dp)).gt.1._dp &
          ...
          if (                                            &
            ...
            if (.not.moreverbose(ind%moreverbose%p)%sparse) then
              ...
              do i=1, size(val)
                ...
                ! not recognized datafiles are not readed in
                ! therefore we set their values as nan
                if (all(ubound(model(i)%data) == 0)           &
                  .and..not. model(i)%if_constant_value) then
                  val(i) = setnan()
                else
                  call get_value (                       &
                    model  = model(i),                   &
